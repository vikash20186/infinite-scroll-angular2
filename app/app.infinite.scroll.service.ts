...
export class InfiniteScrollService {
  getInfiniteScrollServiceObject(){
...
function InfiniteScroll(){
    ...
    //Initialization methods
		...
		function initializeProperties(){
		  ...
		  var totalHeight = settings.dataList.length * settings.rowHeight;
    ...
    //Removing & Attaching Scroll events. Can use Rx evnts also. 
		...
		function handleScrollDown(evt){
			if (rowElems && rowElems.length){
				...
				if (clientRect.bottom < 0){
					...
					if( lastIndexNumber+1 < (settings.dataList || []).length){
    ...
    function handleScroll(evt, isScrollDown){
		...
		function  removeAllChilds(upperCount, lowerCounter, childsParent, childs){
		  var dataIndexNumbers = [];
			for (;upperCount>=lowerCounter;upperCount--){
				if (childsParent.length === 0){
					console.error("error here");
				}
				//childsParent.removeChild(childs[upperCount]);
				dataIndexNumbers.push(childs[upperCount].getAttribute("data-indexnumber"));
			}
			//remove requires view to be destroyed.
			settings.removeRowsCallback && settings.removeRowsCallback(dataIndexNumbers);
		}
		...
		function renderPageData(startCount, endCount, isScrollDown, removeAllChild){
			...
			var totalDataLength = (settings.dataList || []).length;
			...
			function renderLists(){
				...
				settings.renderRowsCallback({
				  startCount : startCount,
				  endCount : endCount,
				  appendChild : isScrollDown !== false
				});
				...
				if (settings.afterGridRenderCallback){
					settings.afterGridRenderCallback(rowElems)
				}
			}
			renderLists();
		}
